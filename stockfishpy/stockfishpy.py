...
FEN_FORMAT_REGEX = r"\s*^(((?:[rnbqkpRNBQKP1-8]+\/){7})[rnbqkpRNBQKP1-8]+)\s([b|w])\s([K|Q|k|q|-]{1,4})\s(-|[a-h][1-8])\s(\d+\s\d+)$"

FEN_DIGITS = frozenset(["1", "2", "3", "4", "5", "6", "7", "8"])
FEN_LETTERS = frozenset("".join([c.lower() + c.upper() for c in "pnbrqk"]))
...
class Engine(subprocess.Popen):
    ...
    def __init__(
        ...
        stockfish_path: str,
        depth: str | int = 12,
        param: Optional[Dict[str, str | int]] = None,
    ):
        if param is None:
            param = {}
    ...
    def setoption(self, optionname: str, value):
    ...
    def setposition(self, position: str | List[str]):
        """
        ...
        try:
            if isinstance(position, list):
                start_pos = " ".join(position).strip()
                self.send("position startpos moves {}".format(start_pos))
                self.isready()
            elif re.match(FEN_FORMAT_REGEX, position):
                regex_list = re.match(FEN_FORMAT_REGEX, position).groups()
                fen = regex_list[0].split("/")
                if len(fen) != 8:
                    raise ValueError(
                        "expected 8 rows in position part of fen: " + repr(fen)
                    )
                    ...
                    for c in fen_part:
                        if c in FEN_DIGITS:
                            if previous_was_digit:
                                raise ValueError(
                                    "two subsequent digits in position part of fen: "
                                    + repr(fen)
                                )
                            field_sum += int(c)
                            ...
                            if not previous_was_piece:
                                raise ValueError(
                                    "~ not after piece in position part of fen: "
                                    + repr(fen)
                                )
                            previous_was_digit, previous_was_piece = False, False
                        elif c in FEN_LETTERS:
                            field_sum += 1
                            previous_was_digit = False
                            previous_was_piece = True
                        else:
                            raise ValueError(
                                "invalid character in position part of fen: "
                                + repr(fen)
                            )
        ...
        while True:
            ...
            if line[0] == "bestmove":
                if self.param["Ponder"] == "true":
                    ...
                    ponder = None
                return BestMove(bestmove=line[1], ponder=ponder, info=" ".join(info))
            info = line
